...
###### STEP 2: Downloading validation data from ICOS or KNMI #############
...
def fetch_knmi_data(endpoint, start_date, end_date, api_key):
    ...
    for filename in filenames:
        if filename in file_list:
            result = requests.get(f"{endpoint.rstrip('/')}/{filename}/url", headers=headers)
            r = requests.get(result.json()["temporaryDownloadUrl"])
            r.raise_for_status()
            with tempfile.NamedTemporaryFile(suffix=".nc") as f:
                f.write(r.content)
                f.flush()
                ds = xr.open_dataset(f.name).drop_vars("valid_dates", errors="ignore")
            files.append(ds)
        else:
    ...
    # Filter time range
    start_slice = start_date.tz_localize(None) if start_date.tz is not None else start_date
    end_slice = end_date.tz_localize(None) if end_date.tz is not None else end_date
...
#Authentication based on station type
if station_type == "ICOS":
    cookie_path = os.path.join(OSVAS, "icos_cookie.txt")
...
# Fetch raw data based on station type
    if station_type == "ICOS":
        ...
        ds_raw = fetch_knmi_data(doi, start_date, end_date, api_key)
# Convert the KNMI dataset into a pandas DataFrame while preserving time.
        df_raw = ds_raw.to_dataframe().reset_index()
        if "time" not in df_raw.columns:
            raise RuntimeError("KNMI dataset conversion did not produce a time coordinate.")

        if df_raw["time"].dt.tz is None:
        ...
        df_raw["valid_dttm"] = df_raw["TIMESTAMP"].dt.tz_localize(None)
...
# Expand wildcards for ICOS data
    variable_map_raw = ds_info["variables"]
    if station_type == "ICOS":
        variable_map_raw, units_map_ds = expand_wildcard_variables(
            variable_map_raw, ds_info.get("units", {}), df_raw.columns
        )
        units_map.update(units_map_ds)
    else:
        units_map.update({k: v for k, v in ds_info.get("units", {}).items() if v is not None})

    df_processed = process_data(df_raw, variable_map_raw, station_info, start_date, end_date)
    if df_processed.empty:
        raise RuntimeError(
            f"❌ No processed data found in the time window ({start_date} to {end_date}) "
            f"for dataset {ds_name} (DOI: {doi}). Aborting."
        )
...
# ---- insert missing parameter rows ----
        ...
        print(f"✅ Year {year} data merged into {output_file}")
print("✅ Flux downloader completed successfully!")
...
if initialization_data:
    ...
    for ds_name, ds_info in init_datasets.items():
        ...
        if station_type == "ICOS":
            ...
            ds_raw = fetch_knmi_data(ds_info["doi"], init_start, init_end, api_key)
            df_raw = ds_raw.to_dataframe().reset_index()
            if "time" not in df_raw.columns:
                raise RuntimeError("KNMI dataset conversion did not produce a time coordinate.")
